...
void main() {
  ...
  diffuse.rgb = mix(vec3_splat(luminance(diffuse.rgb)), diffuse.rgb, 1.08);
  diffuse.rgb += glow*0.85;

  if (v_extra.b > 0.9) {
    diffuse.rgb += v_refl.rgb*(v_refl.a*0.85);
    ...
    if (dy < 0.0002) {
      ...
      diffuse.rgb *= 1.0 - 0.45*mask;
      diffuse.rgb += v_refl.rgb*(mask*0.75);
    }
  }

  diffuse.rgb = mix(diffuse.rgb, v_fog.rgb, smoothstep(0.0, 1.0, v_fog.a));
  
  // NewB x Vellum Ambient Lift
  diffuse.rgb += vec3(0.008, 0.010, 0.012);
