...
void main() {
  ...
  fogColor.a = nlRenderFogFade(relativeDist, FogColor.rgb, FogAndDistanceControl.xy);
    ...
    fogColor.a = mix(fogColor.a, 1.0, min(NL_GODRAY*1.15*nlRenderGodRayIntensity(cPos, worldPos, t, uv1, relativeDist, FogColor.rgb), 1.0));
  #endif
  ...
  vec4 pos = mul(u_viewProj, vec4(worldPos, 1.0));
    ...
    if (env.rainFactor > 0.0) {
      ...
      fogColor.a = mix(fogColor.a, 1.0, humidAir*NL_RAIN_MIST_OPACITY*0.75
